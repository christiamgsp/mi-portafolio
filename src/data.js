export const perfil = {
  nombre: 'Christiam Gabriel Silva Parra',
  titulo: 'Frontend Engineer | AI Specialist | Bilingual (English C1)',
  frase:
    '10+ años de experiencia en IT evolucionando hacia el desarrollo Frontend. Especialista en ecosistemas React y optimización de flujos de trabajo con Inteligencia Artificial.',
  linkedin: 'https://linkedin.com/in/tu-perfil',
  github: 'https://github.com/christiamgsp',
  foto: '/pdfs/foto-perfil.jpg',
};

export const certificaciones = [
  {
    id: 1,
    titulo: 'Claude IA para Developers',
    academia: 'Platzi',
    fecha: 'Marzo 2026',
    link: '/pdfs/diploma-claude.pdf',
    categoria: 'AI',
  },
  {
    id: 2,
    titulo: 'Herramientas de AI para Developers',
    academia: 'Platzi',
    fecha: 'Feb 2026',
    link: '/pdfs/diploma-herramientas-ai-developers-2024.pdf',
    categoria: 'AI',
  },
  {
    id: 3,
    titulo: 'Prompt Engineering',
    academia: 'Platzi',
    fecha: 'Feb 2026',
    link: '/pdfs/diploma-prompt-engineering.pdf',
    categoria: 'AI',
  },
  {
    id: 4,
    titulo: 'React Avanzado',
    academia: 'Platzi',
    fecha: 'Oct 2025',
    link: '/pdfs/diploma-react-avanzado.pdf',
    categoria: 'React',
  },
  {
    id: 5,
    titulo: 'React Server Side Rendering (SSR)',
    academia: 'Platzi',
    fecha: 'Oct 2025',
    link: '/pdfs/diploma-react-ssr.pdf',
    categoria: 'React',
  },
  {
    id: 6,
    titulo: 'React Testing Library',
    academia: 'Platzi',
    fecha: 'Junio 2025',
    link: '/pdfs/diploma-react-testing-library.pdf',
    categoria: 'React',
  },
  {
    id: 7,
    titulo: 'React con TypeScript',
    academia: 'Platzi',
    fecha: 'Mayo 2024',
    link: '/pdfs/diploma-react-typescript.pdf',
    categoria: 'React',
  },
  {
    id: 8,
    titulo: 'Frontend Developer',
    academia: 'Platzi',
    fecha: 'Marzo 2023',
    link: '/pdfs/diploma-frontend-developer.pdf',
    categoria: 'Frontend',
  },
  {
    id: 9,
    titulo: 'Tailwind CSS',
    academia: 'Platzi',
    fecha: 'Agosto 2025',
    link: '/pdfs/diploma-tailwind-css.pdf',
    categoria: 'Design',
  },
  {
    id: 10,
    titulo: 'Figma: Sistemas de Diseño',
    academia: 'Platzi',
    fecha: 'Enero 2026',
    link: '/pdfs/diploma-figma-sistemas-diseno.pdf',
    categoria: 'Design',
  },
  {
    id: 11,
    titulo: 'Git y GitHub Profesional',
    academia: 'Platzi',
    fecha: 'Junio 2025',
    link: '/pdfs/diploma-gitgithub.pdf',
    categoria: 'Tools',
  },
  {
    id: 12,
    titulo: 'Angular Framework',
    academia: 'Platzi',
    fecha: '2026',
    link: '/pdfs/angular.pdf',
    categoria: 'Frontend',
  },
  {
    id: 13,
    titulo: 'Node.js Mastery',
    academia: 'Platzi',
    fecha: '2026',
    link: '/pdfs/node.pdf',
    categoria: 'Backend',
  },

  {
    id: 14,
    titulo: 'Redux Profesional',
    academia: 'Platzi',
    fecha: '2026',
    link: '/pdfs/diploma-react-redux-profesional.pdf',
    categoria: 'React',
  },
  {
    id: 15,
    titulo: 'Vue.js',
    academia: 'Platzi',
    fecha: '2026',
    link: '/pdfs/diploma-testing-vuejs.pdf ',
    categoria: 'Frontend',
  },
];
export const proyectos = [
  {
    id: 1,
    nombre: 'MediStock (Angular)',
    desc: 'Sistema de gestión farmacéutica con control de stock visual, filtros inteligentes y persistencia de datos.',
    stack: ['Angular 18', 'TypeScript', 'LocalStorage', 'CSS3'],
    github: 'https://github.com/christiamgsp/MediStock',
    link: 'https://medi-stock-gamma.vercel.app/',
  },
  {
    id: 2,
    nombre: 'DataPulse',
    desc: 'Dashboard interactivo de visualización de datos dinámicos mediante gráficos avanzados.',
    stack: ['React', 'D3.js', 'Tailwind', 'TS'],
    github: 'https://github.com/christiamgsp/data-pulse',
    link: 'https://data-pulse-ivory.vercel.app/',
  },
  {
    id: 3,
    nombre: 'SmartShop E-commerce + IA',
    desc: 'E-commerce bilingüe con agentes de IA integrados para soporte y recomendaciones personalizadas.',
    stack: ['React', 'OpenAI API', 'Tailwind', 'TS'],
    github: 'https://github.com/christiamgsp/modern-ecommerce-react-IA',
    link: 'https://modern-ecommerce-react-gwkpjtg7u-christiamgsps-projects.vercel.app/',
  },
  {
    id: 4,
    nombre: 'OrganizeMe',
    desc: 'Gestor de tareas avanzado (Clon de Notion) con persistencia y lógica de estados complejos.',
    stack: ['React', 'Tailwind', 'Testing'],
    github: 'https://github.com/christiamgsp/OrganizeMe',
    link: 'https://organize-me-gules.vercel.app/',
  },
  {
    id: 5,
    nombre: 'TareApp',
    desc: 'Aplicación optimizada para la gestión de tareas diarias con enfoque en UX minimalista.',
    stack: ['React', 'LocalStorage', 'CSS'],
    github: 'https://github.com/christiamgsp/tareapp',
    link: 'https://christiamgsp.github.io/tareapp/',
  },
];
